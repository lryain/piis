...
#define DC_PIN    25             // These pins connect
#define RESET_PIN 16             // to BOTH screens
#define DCMASK    (1 << DC_PIN) // GPIO pin bitmasks of reset + D/C pins
#define RESETMASK (1 << RESET_PIN)
#define PWM_PIN 12             // to BOTH screens
#define PWMMASK (1 << PWM_PIN)
...
initIPS[] = {
  ...
  0xFE, 0x80,       255,      // 寄存器使能
  0xEF, 0x80,       255,      //
  0x84,    1, 0x40,           // 删掉则是镜像模式，1 arg (RGB), no delay,
  0xB6,    2, 0x00, 0x20,      // 屏幕功能配置，2 arg, 10ms delay

  // 扫描模式 内存访问控制 USE_HORIZONTAL==0
  0x36,    1, 0x08,           // MADCTL, 1 arg (RGB), no delay,
  // 颜色模式选择
  0x3A, 0x81, 0x05,  10,      // COLMOD, 1 arg, 10ms delay
  // 电源控制
  0xC3,    1, 0x13,     // Pwr ctrl 3
  0xC4,    1, 0x13,     // Power control 4, 1 args, no delay
  0xC9,    1, 0x22,     // Power control 9, 1 args, no delay

  //伽马设置
  0xF0,    6,                 // SET_GAMMA set, 6 args, no delay:
    0x45, 0x09, 0x08, 0x08, 0x26, 0x2A,
  0xF1,    6,                 // SET_GAMMA set, 6 args, no delay:
    0x43, 0x70, 0x72, 0x36, 0x37, 0x6F,
  0xF2,    6,                 // SET_GAMMA set, 6 args, no delay:
    0x45, 0x09, 0x08, 0x08, 0x26, 0x2A,
  0xF3,    6,                 // SET_GAMMA set, 6 args, no delay:
    0x43, 0x70, 0x72, 0x36, 0x37, 0x6F,

  //帧率 Frame Rate (E8h)
  0xE8,    1, 0x34,           // Frame Rate (E8h), 1 arg, no delay

  // 其他寄存器
  0x66,   10,                 // ???, 10 args, no delay:
    0x3C, 0x00, 0xCD, 0x67, 0x45,
    0x45, 0x10, 0x00, 0x00, 0x00,
  0x67,   10,                 // ???, 10 args, no delay:
    0x00, 0x3C, 0x00, 0x00, 0x00,
    0x01, 0x54, 0x10, 0x32, 0x98,
  // 亮度
//   0x51, 0x80,       150,      // Soft reset, no args, 150 ms delay
  // 剪切效果线打开，关闭是 0x34
  0x35, 0x80,        10,      // INVON, no args, 10 ms delay
  // 颜色反转  20 是关闭，21 是打开
  0x21, 0x80,        10,      // INVON, no args, 10 ms delay
  // 退出休眠模式，10 是睡眠模式
  0x11, 0x80,       255,      // DISPON, no args, 500 ms delay
  // 开始显示
...
int main(int argc, char *argv[]) {
	...
	INP_GPIO(PWM_PIN); OUT_GPIO(PWM_PIN);

	*gpioSet = PWMMASK; usleep(5);   // PWM high,
